  ...
  const handleCTA = () => {
    ...
    switch (action) {
      case 'update_opportunity':
        navigate(`/opportunities?selected=${entityId}`);
        break;
      case 'review_grant':
        navigate(`/grants?selected=${entityId}`);
        break;
      case 'open_anchor':
        navigate(`/anchors?selected=${entityId}`);
        break;
      case 'open_pipeline':
        navigate(`/pipeline?selected=${entityId}`);
        break;
      case 'open_event':
        navigate(`/events?selected=${entityId}`);
        break;
      case 'open_bundle_panel':
        // Navigate to settings with AI section open
