...
export default function CommunioRequestsTab() {
  ...
  const { data: replies } = useQuery({
    queryKey: ['communio-replies', expandedId],
    queryFn: async () => {
  ...
  const createRequest = useMutation({
    ...
    onSuccess: () => {
      ...
      setCategory('general');
      setShowForm(false);
      qc.invalidateQueries({ queryKey: ['communio-requests'] });
    },
  ...
  const createReply = useMutation({
    ...
    onSuccess: () => {
      ...
      setReplyText('');
      qc.invalidateQueries({ queryKey: ['communio-replies', expandedId] });
    },
