...
export default function Events() {
  ...
  const defaultMetroId = metroEnabled ? metros?.find(m => m.metro.toLowerCase().includes('minneapolis'))?.id : undefined;
  if (metroEnabled && metros && metroFilter === null) {
    setMetroFilter(defaultMetroId || 'all');
  } else if (!metroEnabled && metroFilter === null) {
    setMetroFilter('all');
  }
  ...
  useEffect(() => {
    ...
    if (detailId) {
      navigate(`/events/${detailId}`, { replace: true });
    }
  ...
  const handleRowClick = (event: Event) => {
    navigate(isOperatorRoute ? `/operator/events/${event.id}` : `/events/${event.id}`);
  };
